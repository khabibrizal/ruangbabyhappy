...
export type PaketRow = {
  ...
  harga: number; diskon_returning: number; dp_persen: number; durasi_menit: number;
  bisa_studio: boolean; bisa_home: boolean; is_active: boolean;
};
...
export async function listPaket(): Promise<PaketRow[]> {
  ...
  const { data } = await admin
    .from("package")
    .select("id, layanan_id, nama, deskripsi, harga, diskon_returning, dp_persen, durasi_menit, bisa_studio, bisa_home, is_active")
    .order("harga");
